...
| Expand ingestion regression coverage | Grow sanitized fixture coverage and live subset checks as more slices are enabled. | `tests/fixtures/`, `tests/test_fixture_ingestion.py`, `tests/test_live_ingestion.py` |
| Improve replay and support workflows | Make issue reporting, support-bundle handling, and replay workflows easier for operators. | `forward_nautobot/integrations/forward/support.py`, `forward_nautobot/management/commands/forward_dry_run.py`, `README.md` |
...
| Reduce query load and request pressure | Minimize unnecessary Forward API calls and keep request shapes explicit and efficient. | `forward_nautobot/integrations/forward/client.py`, `forward_nautobot/integrations/forward/planner.py`, query contracts |
...
4. Support and replay artifacts are safe to share and easy to act on.
